...
public class MercuryNewScheme<Integer> extends AbstractScheme<Integer>
{
	...
	public MercuryNewScheme(InterfaceNodeConfig<Integer> nc, JSONMessenger<Integer> m)
	{
		super(nc, m);
			...
			processQueryMsgFromUser(QueryMsgFromUser<Integer> queryMsgFromUser)
	{
		...
		QueryInfo<Integer> currReq = new QueryInfo<Integer>(query, getMyID(), grpGUID, 
			...
			processQueryMsgToMetadataNode(QueryMsgToMetadataNode<Integer> queryMsgToMetaNode)
	{
		...
		return (GenericMessagingTask<Integer, QueryMsgToValuenode<Integer>>[]) 
			...
			processQueryMsgToValuenode(QueryMsgToValuenode<Integer> queryMsgToValnode)
	{
		...
		synchronized( reqResClass.resultArray )
		{
			...
			{
				//ContextServiceLogger.getLogger().fine("reqResClass wakes up "+reqResClass.getNumGuidsCompl()
		...
		QueryMsgToValuenodeReply<Integer> queryMsgToValReply 
			= new QueryMsgToValuenodeReply<Integer>(getMyID(), reqResClass.getResultArray(), requestID, 
					...
					try 
					{
						...
						if( guidObject != null )
						{
							try
							{
								...
								{
									//ContextServiceLogger.getLogger().fine(reqdObject);
									flag = true;
								}
								synchronized(resultArrayObject.resultArray)
								{
									//ContextServiceLogger.getLogger().fine("Notify "+guidToGET+" "+resultArrayObject.getNumGuidsCompl());
									if(flag)
									{
