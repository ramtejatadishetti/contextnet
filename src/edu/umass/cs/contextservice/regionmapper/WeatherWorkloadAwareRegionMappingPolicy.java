...
public class WeatherWorkloadAwareRegionMappingPolicy extends AbstractRegionMappingPolicy
{
	...
	private static final String SEARCH_TRACE_FILE					= "traces/weatherTrace/searchFile.txt";
	//private static final String UPDATE_TRACE_FILE					= "traces/weatherTrace/updateFile.txt";
	
	private final LinkedList<RegionInfo> regionList;
	
	
	public WeatherWorkloadAwareRegionMappingPolicy(HashMap<String, AttributeMetaInfo> attributeMap, 
...
//					splitRegion2.setTraceLoad(currLoad2);
	...
	private double computeUpdateLoadOnARegionBasedOnTrace( RegionInfo regionInfo, 
			HashMap<String, AttributeMetaInfo> attributeMap, int totalNodes )
	{
		double regionLogVol = this.computeLogVolume
				(regionInfo.getValueSpaceInfo().getValueSpaceBoundary(), 
						attributeMap);
		
		HashMap<String, AttributeValueRange> attrValMap 
		...
		while(attrIter.hasNext())
		{
			...
			attrValMap.put(attrName, attrValRange);
		}
		
		double totalLogVol = this.computeLogVolume(attrValMap, attributeMap);
		
		double updateRequestProb = Math.exp(regionLogVol)/Math.exp(totalLogVol);
...
//		}
	...
	{		
		...
		AttributeMetaInfo attrInfo 
			= new AttributeMetaInfo("latitude", 41.8+"", 45.1+"", AttributeTypes.DoubleType);
		
		givenMap.put(attrInfo.getAttrName(), attrInfo);	
		attrList.add(attrInfo.getAttrName());
		
		attrInfo 
			= new AttributeMetaInfo("longitude", -80.0+"", -74.5+"", AttributeTypes.DoubleType);
...
//		attrList.add(attrInfo.getAttrName());
		...
		for(int n=0; n<nodeList.length; n++)
		{
			...
			WeatherWorkloadAwareRegionMappingPolicy obj 
					= new WeatherWorkloadAwareRegionMappingPolicy(givenMap, csNodeConfig);
