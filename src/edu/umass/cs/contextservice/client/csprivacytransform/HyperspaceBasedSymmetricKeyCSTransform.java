 ...
 *
 */
public class HyperspaceBasedSymmetricKeyCSTransform implements CSPrivacyTransformInterface
{	
	private final ExecutorService exectutorService;
	
	public HyperspaceBasedSymmetricKeyCSTransform(ExecutorService execService)
	{
	...
	{
		ParallelSearchReplySymmetricKeyDecryption parallelSearchDecryption =
				new ParallelSearchReplySymmetricKeyDecryption(myGuid , csTransformedList
			...
			List<CSSearchReplyTransformedMessage> csTransformedList, JSONArray replyArray) 
	{
		...
		for( int i=0; i<csTransformedList.size(); i++ )
		{
					...
					try
					{
						try
						{
							byte[] guidBytes = Utils.doSymmetricDecryption( symmetricKey, 
									...
									(anonymizedIDToGUIDMapping.getString(0)) );
						...
						{
							// wrong padding only happens on using wrong key.
							// so catching the exceptions 
						}
					}
			...
			{
				// if anonymizedIDToGUDIMapping is null then it could be a no privacy GUID.
				replyArray.put(anonymizedID);
			}
