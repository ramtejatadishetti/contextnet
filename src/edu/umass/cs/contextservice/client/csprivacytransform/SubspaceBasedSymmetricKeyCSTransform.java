...
{	
			...
			List<CSSearchReplyTransformedMessage> csTransformedList, JSONArray replyArray) 
	{
		...
		unTransformSearchReplyInternal(anonymizedIDToSecretKeyMap,
				csTransformedList, replyArray, null);
	}
			...
			List<CSSearchReplyTransformedMessage> csTransformedList, HashMap<String, Boolean> replyMap) 
	{
		...
		unTransformSearchReplyInternal(anonymizedIDToSecretKeyMap,
				csTransformedList, null, replyMap);
	}
	
	
	private void unTransformSearchReplyInternal(HashMap<String, byte[]> anonymizedIDToSecretKeyMap,
			List<CSSearchReplyTransformedMessage> csTransformedList, JSONArray replyArray, 
			HashMap<String, Boolean> replyMap)
	{	
		for( int i=0; i<csTransformedList.size(); i++ )
		{
				...
				if( anonymizedIDToSecretKeyMap.containsKey(anonymizedID) )
				{
					...
					try
					{
						try
						{
							...
							if(replyMap != null)
								replyMap.put(Utils.byteArrayToHex(guidBytes), true);
						}
			...
			else
			{
				...
				if(replyMap != null)
					replyMap.put(anonymizedID, true);
			}	
		}
	}
